...
public struct RTSRouterRootView: View {

    @EnvironmentObject private var vm: RTSRouterViewModel
    ...
    @StateObject private var gate = RTSReadyGateHolder()
    ...
    private var splashLayer: some View {
        if let splash = RTSScreenRouterKit.shared.config?.splashProvider {
            splash {
                gate.value.splashReady()
                RTSScreenRouterKit.shared.splashSignal.complete()
            }
        } else {
    ...
    private var content: some View {
        ...
        case .main:
            if let mainProvider = RTSScreenRouterKit.shared.mainViewProvider {
                mainProvider()
            } else {
                Color(.systemBackground).ignoresSafeArea()
            }
        ...
        case .web(let url):
            RTSWebContainerView(url: url)
                .onAppear {
                    RTSOrientationProxy.shared.set(
                        RTSScreenRouterKit.shared.config?.webOrientations ?? .all
                    )
                }
                .onDisappear {
                    RTSOrientationProxy.shared.set(
                        RTSScreenRouterKit.shared.config?.defaultOrientations ?? .portrait
                    )
                }
        }
    }
    ...
    private func fadeOutSplash() {
        ...
        let config = RTSScreenRouterKit.shared.transitionConfig
...
@MainActor
private final class RTSReadyGateHolder: ObservableObject {
    let value = RTSReadyGate()
}

public final class RTSOrientationProxy {

    public static let shared = RTSOrientationProxy()
    private init() {}
    ...
    public func set(_ mask: UIInterfaceOrientationMask) {
        ...
        RTSLogger.log(.debug, "Orientation: \(mask.rawValue)")
    }
}
